...
function setupSite() {
  ...
  site.forbidsScript = window.forceNoScript || false;
  ...
  site.init = function() {
    if (site.openedBySearchbot || site.forbidsScript) return;
...
function setupSiteFrame() {
  ...
  frame.setFontSize = function(fontSize) {
    ...
    const old_style = document.head.querySelector('#lyt_font_size_style');
    if (fontSize == 1) {
      if (old_style) old_style.remove();
      return;
    }
    if (old_style && fontSize == old_style.fontSize) return;
    const css = 'body { font-size: ' + fontSize + 'em }';
    if (old_style) { old_style.textContent = css; old_style.fontSize = fontSize; return; }
    const new_style = document.createElement('style');
    new_style.id = 'lyt_font_size_style';
    new_style.textContent = css;
